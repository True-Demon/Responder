...
parser.add_option('-f', '--force-smb', action="store_true", help="Create SMB listener on Windows", metavar="whoami",dest="forceSMB")
...
def ConnectToTarget():
		try:
			...
			try:
				print "Cannot connect to target, host down? " + str(e)
				sys.exit(1)
...
def main():
	 try:
		threads = []
		if (isWin and forceSMB) or not isWin:
